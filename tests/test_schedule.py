...
class ScheduleTest(unittest.TestCase):
    ...
    def test_removing_class_not_in_schedule_doesnt_change_credits(self):
        class_to_try_to_remove = CourseList.get_courses_using_course_code("CMSC250").sections["0101"]  # getting section 0101
        schedule = MySchedule()
        self.assertEqual(schedule.total_credits, 0)
    ...
    def test_add_class_and_credits_and_schedule_correct(self):
        class_to_add = CourseList.get_courses_using_course_code("CMSC250").sections["0101"]
        schedule = MySchedule()
        self.assertEqual(schedule.total_credits, 0)
    ...
    def test_add_duplicate_class_and_credits_and_schedule_correct(self):
        class_to_add = CourseList.get_courses_using_course_code("CMSC250").sections["0305"]
        schedule = MySchedule()
        self.assertEqual(schedule.total_credits, 0)
        schedule.add_class(class_to_add)
    ...
    def test_add_two_classes_correct_credits_and_schedule(self):
        cmsc250 = CourseList.get_courses_using_course_code("CMSC250").sections["0305"]
        comm107 = CourseList.get_courses_using_course_code("COMM107").sections["0101"]
        schedule = MySchedule()
        self.assertEqual(schedule.total_credits, 0)
    ...
    def test_add_two_classes_make_sure_ordered_by_time(self):
        ...
        schedule.add_class(cmsc250)
        schedule.add_class(comm107)
        self.assertEqual(schedule.schedule["M"][1].section_id, "CMSC250-0305")
        self.assertEqual(schedule.schedule["M"][0].section_id, "COMM107-0101")
        self.assertEqual(schedule.total_credits, 7)

    def test_try_to_add_overlapping_classes_only_first_gets_added(self):
        comm107_0301 = CourseList.get_courses_using_course_code("COMM107").sections["0301"]
        comm107_0101 = CourseList.get_courses_using_course_code("COMM107").sections["0101"]
        schedule = MySchedule()
        schedule.add_class(comm107_0101)
        self.assertEqual(schedule.total_credits, 3)
        schedule.add_class(comm107_0301)  # can't add this class
        self.assertEqual(schedule.total_credits, 3)
        self.assertEqual(schedule.schedule["M"][0].section_id, "COMM107-0101")
    ...
    def test_class_overlap_same_class(self):
        cmsc250 = CourseList.get_courses_using_course_code("CMSC250").sections["0101"]
        cmsc250_2 = CourseList.get_courses_using_course_code("CMSC250").sections["0101"]
        schedule = MySchedule()
        schedule.add_class(cmsc250)
        self.assertFalse(schedule.no_class_overlap(cmsc250_2))

    def test_class_overlap_different_class_time_conflict(self):
        chem131 = CourseList.get_courses_using_course_code("CHEM131").sections["1425"]
        comm107 = CourseList.get_courses_using_course_code("COMM107").sections["0101"]
        schedule = MySchedule()
        schedule.add_class(chem131)
    ...
    def test_try_to_add_class_in_middle_of_day_start_time_overlaps_with_previous_end_time(self):
        cmsc250 = CourseList.get_courses_using_course_code("CMSC250").sections["0101"]
        anth221 = CourseList.get_courses_using_course_code("ANTH221").sections["FC01"]
        chem271 = CourseList.get_courses_using_course_code("CHEM271").sections["2347"]
        schedule = MySchedule()
        schedule.add_class(cmsc250)
...
class ScheduleWarningTest(unittest.TestCase):
    ...
    def test_add_class_no_open_seats_makes_warning(self):
        ...
        full_sections = [section for sections in
                         [course.sections.values() for course in CourseList.get_courses_using_page_number(1).values()]
